...
async function verifyParsing() {
  try {
    console.log('Verifying question parsing...');
    ...
    console.log(`Found ${questions.length} questions. Verifying parsing...\n`);
    
    let format1Count = 0; // With reference to format
    let format2Count = 0; // Consider the following statements format
    let format3Count = 0; // Which of the following format
    let defaultCount = 0; // Default format
    ...
    questions.forEach((question, index) => {
      ...
      console.log(`  Format: ${parsed.type}`);
      
      switch (parsed.type) {
        case 'withReference':
          format1Count++;
          console.log(`  Topic: ${parsed.topicLine}`);
          console.log(`  Statements: ${parsed.statements.length}`);
          console.log(`  Options: ${parsed.options.length}`);
          break;
        case 'considerStatements':
          format2Count++;
          console.log(`  Statements: ${parsed.statements.length}`);
          console.log(`  Options: ${parsed.options.length}`);
          break;
        case 'whichOfFollowing':
          format3Count++;
          console.log(`  Question: ${parsed.questionText.substring(0, 50)}...`);
          console.log(`  Options: ${parsed.options.length}`);
          break;
        case 'default':
          defaultCount++;
          console.log(`  Text: ${parsed.questionText.substring(0, 50)}...`);
          break;
      }
    ...
    console.log('Summary:');
    console.log(`  "With reference to" format: ${format1Count}`);
    console.log(`  "Consider the following statements" format: ${format2Count}`);
    console.log(`  "Which of the following" format: ${format3Count}`);
    console.log(`  Default format: ${defaultCount}`);
