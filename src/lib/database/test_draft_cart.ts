...
async function testDraftCartFunctionality() {
  try {
    ...
    const testManagementStmt = db.prepare(`
        ...
        `);
    const savedTestInManagement = testManagementStmt.get(
      testName,
      batch,
      date,
      defaultUserId,
    ...
    console.assert(
      savedDraftCart.test_name === testName,
    ...
    console.assert(
      savedTestInManagement.status === "draft",
      "Test status is not draft",
    );
    console.assert(
      JSON.parse(savedTestInManagement.questions).toString() ===
