...
function isQuestionResult(q: unknown): q is QuestionResult {
  return (
    q !== null && typeof q === 'object' && 'Question' in q && 'Answer' in q && 'Explanation' in q
  );
}
...
const hierarchicalData: Subject[] = [
  {
    ...
    modules: [
      ...
      },
      {
        ...
        topics: [{ name: 'Population' }, { name: 'Settlements' }, { name: 'Economic Activities' }],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    modules: [
      ...
      },
      {
        ...
        topics: [{ name: 'Executive' }, { name: 'Legislature' }, { name: 'Judiciary' }],
      },
    ],
  },
  {
    ...
    modules: [
      ...
      {
        name: 'Biology',
        topics: [{ name: 'Cell Biology' }, { name: 'Genetics' }, { name: 'Evolution' }],
      },
    ],
  },
];
...
export async function getCartQuestions(testId: string): Promise<QuestionResult[]> {
  if (!testId) {
  ...
  try {
    ...
    const query = `
            ...
            WHERE c.test_id = ?
        `;
    const cartQuestions = (await db.prepare(query).all(testId)) as QuestionResult[];
...
export async function updateQuestion(question: QuestionResult): Promise<QuestionResult> {
...
export async function addQuestion(question: QuestionResult): Promise<{ id: number }> {
  const db = await openDatabase();
...
export async function ensureDefaultUser() {
  ...
  try {
    ...
    if (userCount.count === 0) {
      ...
      // Insert default user
      const result = stmt.run('defaultuser', 'default@example.com', hashedPassword, 'user', 1);
...
export async function openDatabase() {
  ...
  try {
    ...
    console.warn('Index creation might have failed (possibly already exists):', indexError);
  }
...
export async function debugQuestionsTable() {
  ...
  try {
    ...
    const sampleQuestions = await db
      .prepare('SELECT Subject, "Module Name", Topic, Question_Type FROM questions LIMIT 10')
      .all();
...
export async function saveDraftCart(
  ...
  try {
    ...
    // Validate user
    let userCheck = db.prepare('SELECT id, email FROM users WHERE id = ?').get(userId);
